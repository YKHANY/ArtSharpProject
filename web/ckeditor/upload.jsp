...
<%
	...
	if (isMultipart) {
		// 설정단계
 		File temporaryDir = new File("../img/tmp");  //업로드된 파일의 임시 저장 폴더
        ...
        System.out.println("CKEditorFuncNum " + sFunc);
		...
		while (iter.hasNext()) {
			...
			if (fileItem.isFormField()) {  // File 컴포넌트가 아닌 일반 컴포넌트일 경우
				...
				if (fileItem.getSize() > 0) {  //파일이 업로드 되었나 안되었나 체크
                    ...
                    String fileName = df.format(new Date()) + filetype;
			        ...
			        out.println("[sizeInBytes] : "+ sizeInBytes +"<br/>");
			 		...
			 		out.println("<script type='text/javascript'>window.parent.CKEDITOR.tools.callFunction(" + sFunc + ", '"+ realUrl + fileName + "', '완료! 이미지 정보탭으로 이동하세요');</script>");
